...
@Form.create()
export default class PrintingProfile extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
      ...
      address: props.dataSource.address,
      weight: props.dataSource.weight,
      needpay: props.dataSource.needpay,
      id: props.dataSource.id,
      phone : props.dataSource.shipphone,
      date : props.dataSource.date,
      company:props.dataSource.company,
      nums: props.dataSource.nums,
      piecenum: props.dataSource.piecenum,
      taxstnum: props.dataSource.taxstnum,// 发票税收
      securepay: props.dataSource.securepay,// 运费
      shippay: props.dataSource.shippay,// 运费
    };
  }
    ...
    const columns = [
      {
        ...
        render: (val, record, index) => {
          if(record.kindname === undefined){
            ...
            return (
              <div>
                <li>{`${record.kindname}${record.seriesname}`}</li>
              </div>
            );
          }
        ...
        <div id={'billDetails'} style={{ border: 'solid' }} className="Gdetails">
          <Card bordered={false} title="">
            <div style={{ position: 'relative' }}>
              <img src={`${company.logo}`} alt="" className="CompanyLogo" />

              <div style={{ display: 'flex' }}>
                <h2 style={{ width:600, flex: 1, textAlign: 'center', marginBottom: '0' }}>
                  {`${company.fullname}销售单`}
                </h2>
                <div>
              ...
              <div style={{ display: 'flex', fontSize: '10px' }} className="TaiTou">
                <h3 style={{ flex: 2, textAlign: 'left' }}>{`购货单位:${this.state.user}`}</h3>
                <h3 style={{ flex: 4, textAlign: 'left' }}>{`日期:${moment(this.state.date).format(
                  'YYYY-MM-DD'
                )}`}</h3>
              </div>
              <div style={{ display: 'flex', fontSize: '10px' }} className="TaiTou">
                <h3 style={{ flex: 2, textAlign: 'left' }}>{`电话:${this.state.phone}`}</h3>
          ...
          <Card bordered={false} title="" bodyStyle={{ padding: '0 16px' }} className="Gtable">
            <Table className={stlyes.Gtable} style={{backgroundColor:'#FFFFFFF'}} dataSource={this.state.data} columns={columns} bordered pagination={false} />
          </Card>
          ...
          >
            <div style={{ display: 'flex', borderBottom: '1px solid' }}>
              <h3 style={{ flex: 2, textAlign: 'left' }}>
                {`合计人民币大写:${digitUppercase(this.state.needpay)}`}
              </h3>
              {/* <h3 style={{ flex: 1 }}>{`重量合计(Kg):${this.state.weight}`}</h3> */}
              <h3 style={{ flex: 1 }}>{`小写(￥):${this.state.needpay}`}</h3>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex' }}>
                ...
                <div className="rightBox" style={{ flex: 1, borderLeft: '1px solid' }} >
                  {`运费:￥${this.state.shippay}`}<br />
                  {`发票(税收):￥${this.state.taxstnum}`}
