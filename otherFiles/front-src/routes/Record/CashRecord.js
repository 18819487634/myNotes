...
function handleDs(obj, pageIndex = 0, pageSize = 10, type) {
  ...
  i+=1;
  params += `&terms[${i}].value=${username}&terms[${i}].column=createid`
  i+=1;
  if (type === 'search') {
    let searchparams = `pageIndex=${index}&pageSize=${pageSize}&sorts[0].name=id&sorts[0].order=desc`;
    Object.keys(obj).forEach(key => {
      if((key==='presaleid') && obj[key] !== undefined){
        /* 
          单号查找 
        */
       searchparams += `&terms[${i}].type=or&terms[${i}].value=${obj[key]}&terms[${i}].column=presaleid`;
        i+=1;
        searchparams += `&terms[${i}].type=or&terms[${i}].value=${obj[key]}&terms[${i}].column=preceiveid`;
        i+=1;
        ...
        */
        if(obj[key] === 2) {
          searchparams += `&terms[${i}].value=-1&terms[${i}].column=validstatus`;
          i+=1;
        } else {
          searchparams += `&terms[${i}].value=${obj[key]}&terms[${i}].column=validstatus`;
          i+=1;
        }
      ...
      */
        searchparams += `&terms[${i}].value=${obj[key]}&terms[${i}].column=paytype`
        ...
        */
        ...
        searchparams += `&terms[${i}].value=${clientid}&terms[${i}].column=clientid`
      }
    });
    return searchparams;
  } else if(type === 'get') {
...
export default class CashRecord extends PureComponent {
  ...
  render() {
    const { cashrecord, loading } = this.props;
        ...
        if((record.createid === username)) {
          ...
          if(record.paytype !== 0) {
            if(status === 0 && (!record.cancelstatus || (record.cancelstatus && record.cancelstatus === 0))) {
              t = ( <Button type="danger" onClick={() => this.handleInvalid(text)}>作废</Button> )
            }
          } else if(record.paytype === 0) {   //  现金
        ...
        <Card>
          <div className={styles.tableList}>
            ...
            <Tabs hideAdd type="editable-card">
              <TabPanes closable={false} tab="收银员">
