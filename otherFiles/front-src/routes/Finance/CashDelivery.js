...
import { Form,  Row, Col,   Input,  Table ,Select,Button,Modal,message, Tabs} from 'antd';
// import PageHeaderLayout from '../../layouts/PageHeaderLayout';
import styles from './AccountNumber.less';
import { getMyDate } from '../../utils/utils';
import TableTop from '../Presale/TableTop';
...
function handleDs(obj, pageIndex = 0, pageSize = 10) {
    let i = 0;
    // 如果填了单号，其它条件都是可以不管的。
    ...
    // 加上类型 nin
  params += `&terms[${i}].value=0&terms[${i}].column=paytype`;
...
export default class CashDelivery extends PureComponent {
  ...
  getDetail = (rec) => {
    ...
    this.props.dispatch({
      type: 'financepaydetail/getPresaleDetail',
      payload: rec,
      callback: (res) => {
        this.updatePanes(res, hide);
      },
    })
  };

  remove = (targetKey) => {
    const activeKeys = this.state.activeKey;
    if(targetKey === activeKeys) {
      this.setState({
        activeKey: '1',
        panes: [],
      })
  ...
  updatePanes = (res, hide) => {
    if(this.state.panes.length > 0) {
      this.setState({
        panes: [],
      }, () => this.showPanes(res, hide))
    } else {
      this.showPanes(res, hide)
    }
  };
      ...
      content: (
        <div>
          {getFieldDecorator('tableTop', {
            initialValue: res.dataTop,
          })(<TableTop />)}
          <div style={{width:"100%",overflowX: "auto"}}>
            {getFieldDecorator('tableLeft')(<TableLeft dataSource={res.dataLeft} />)}
            <div style={{width:500,display:'inline-block'}}>
              {getFieldDecorator('tabletight')(<PickUpTable dataSource={res.dataRight} />)}
            </div>
          </div>
        </div>
      ), 
  ...
  changestatus = (status,rec)=>{
    ...
    if(status===1){
      confirms({
        title: `确认交结这笔${rec.id}收款?`,
        content: ``,
        onOk() {
          const params = {transfer:1, id:rec.id};
          df.props.dispatch({
  ...
  render() {
    ...
    const columntmp = [{
      ...
      render: (text,record) => {
        ...
        return (
          <div>
            <a onClick={() => this.getDetail(record)}>{d}</a>
      ...
      render: (text, record)  => {
        let t = (
          <span style={{color: '#00FF00'}}>已交结</span>
        );
        if(record.cancelstatus === 0 && ((record.transfer && record.transfer === 0) || !record.transfer)){
          t = (
            <div>
              <Button onClick={() =>this.changestatus(1,record)}>交结</Button>
            </div>
           )
        }else if(record.cancelstatus === 1){
          t = (
            <div>
              <span style={{color: '#FF0000'}}>已作废</span>
            </div>
           )
      ...
      <div className={styles.tableList}>
        ...
        <div style={{ margin: '5px' }}>
          <div style={{ width: '100%', float: 'left' }}>
            ...
            >
              <TabPanes tab='现金交结' closable={false} key='1'>
                <Table
                  rowKey="id"
                  loading={loading}
                  dataSource={dataList}
