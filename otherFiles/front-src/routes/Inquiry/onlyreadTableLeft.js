...
import { Table,InputNumber,Select,Input,DatePicker, Checkbox, Radio} from 'antd';
import RadioGroup from 'antd/lib/radio/group';
import moment from 'moment';
import styles from './InquiryProfile.less';

import TableRight from './TableRight';
import { queryGoodsBasic, queryGoods, queryBasicProfile, queryDescBatchno } from '../../services/api';
...
export default class onlyreadTableLeft  extends PureComponent {
...
let prenumsum=0;
  ...
  if(values===0){
    ...
    ownumsum = parseFloat(newConcat[4].ownnum).toFixed(2);
    spotnumsum = parseFloat(newConcat[4].spotnum).toFixed(2);
    prenumsum =parseFloat(newConcat[4].prereceivenum).toFixed(2);
  }else{
    if(newConcat[0].ownnum>0 || newConcat[0].transnum>0 || newConcat[0].rawyarnnum>0){// 有数量才有运费保险
      targetFreight[own] = (firstFreightPrice+(newConcat[0].ownnum-firstFreight<0?0:newConcat[0].ownnum-firstFreight)*continuedFreight).toFixed(2);
      targetInsurance[own] = (firstInsurancePrice+(newConcat[0].ownnum-firstInsurance<0?0:newConcat[0].ownnum-firstFreight)*continuedInsurance).toFixed(2);
    }
    
    if(newConcat[0].spotnum>0){
      targetFreight[spot] = (firstFreightPrice+(newConcat[0].spotnum-firstFreight<0?0:newConcat[0].spotnum-firstFreight)*continuedFreight).toFixed(2);
      targetInsurance[spot] =(firstInsurancePrice+(newConcat[0].spotnum-firstInsurance<0?0:newConcat[0].spotnum-firstFreight)*continuedInsurance).toFixed(2);
    }
    if(newConcat[0].prereceivenum>0){
      targetFreight[pre] = (firstFreightPrice+(newConcat[0].prereceivenum-firstFreight<0?0:newConcat[0].prereceivenum-firstFreight)*continuedFreight).toFixed(2);
      targetInsurance[pre] =(firstInsurancePrice+(newConcat[0].prereceivenum-firstInsurance<0?0:newConcat[0].prereceivenum-firstFreight)*continuedInsurance).toFixed(2);
    }
   

    
    
    ownumsum = (parseFloat(newConcat[4].ownnum)+parseFloat(newConcat[5].ownnum)+parseFloat(newConcat[6].ownnum)).toFixed(2);
    spotnumsum = (parseFloat(newConcat[4].spotnum)+parseFloat(newConcat[5].spotnum)+parseFloat(newConcat[6].spotnum)).toFixed(2);
    prenumsum = (parseFloat(newConcat[4].prereceivenum)+parseFloat(newConcat[5].prereceivenum)+parseFloat(newConcat[6].prereceivenum)).toFixed(2);
  }
 
  const sum = ownumsum+ spotnumsum+prenumsum;
  targetPlan[own] = ownumsum;
  targetPlan[pre] = prenumsum;
...
generalSum(){
  ...
  newData1.chagePriceFlag = this.state.chagePriceFlag;
    ...
    if(prenum>0 && this.state.freightValue ===1){
      Frepre = firstFreightPrice+(spotnum-firstFreight<0?0:spotnum-firstFreight)*continuedFreight;
      Inspre = firstInsurancePrice+(spotnum-firstInsurance<0?0:spotnum-firstFreight)*continuedInsurance;
    }else{
      Frepre =0;
      Inspre = 0;
    }
    ...
    if(spotnum>0 && this.state.freightValue ===1){
     ...
     invoicespotnum = Goodspotnum+Frespotnum+Insspotnum*(1-invoice);
      // 计算税收
      invoiceownnum = Goodsownnum+Freownnum+Insownnum *(1-invoice);
      invoicespre =Goodsprenum+Frepre+Inspre*(1-invoice);
     if(Goodsasenum !==0){
            ...
            return (
              <div>
                <InputNumber
                  ...
                  onChange={e => this.handleFieldChange(e, 'prereceivenum', record.key)}
                />
